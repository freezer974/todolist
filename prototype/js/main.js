...
$("body").on( "click", "button.button-addon", function(event) {
    ...
    if (input.val() === '') {
        ...
        li.addClass('list-group-item d-flex justify-content-between align-items-center animated flipInX');
        ...
        check = createSpanFa('checkbox','fa fa-square text-muted mr-2',{'aria-hidden': 'true'});
        
        badge = createSpanFa('badge badge-primary badge-pill bg-danger animated fadeInRight d-none',  
        'fa fa-times',  
        {'aria-hidden': 'true'}
        );
        ...
        li.append(divCheck).append(badge);
...
$("body").on("click", ".check", function(event) {    
    ...
    $(this).find('.checkbox>i').toggleClass('text-muted fa-check-square text-success animated bounceIn');
    
    //on active ou desactive les class du bouton de suppression des tâches
    $(this).closest('li').find('.badge').toggleClass('d-none animated fadeInRight');
    
});

// on ecoute dans body le click sur la span badge de suppression des tâches
$("body").on("click", ".badge", function(event) {    
...
// on enclanche le carrousel pour la page d'accueil
$('.carousel').carousel();
